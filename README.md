...
cd /I2p_FSpider/docker
修改 scrapyd.conf、scrapyd_monitor.sh  相关配置为你部署服务器地址
docker build -t wholescrapyd:latest .      镜像构建时间可能较长耐心等待
将 /I2p_FSpider/docker/crontab.txt  内容输入crontab -e 设定定时任务
修改 /I2p_FSpider/i2p_spider/settings.py  相关配置
修改 /I2p_FSpider/schedule/config.py  相关配置

创建mapping
cd /I2p_FSpider/schedule/mapping  查看mapping说明按说明执行即可,ip修改为es地址即可
