...
项目简介: 此项目为针对tor的暗网全网web数据分布式采集、分析、存储
...
cd /tor_whole_network/docker
修改 scrapyd.conf、scrapyd_monitor.sh  相关配置为你部署服务器地址
docker build -t wholescrapyd:latest .      镜像构建时间可能较长耐心等待
将 /tor_whole_network/docker/crontab.txt  内容输入crontab -e 设定定时任务
修改 /tor_whole_network/settings.py  相关配置
修改 /tor_whole_network/schedule/config.py  相关配置

创建mapping
cd /tor_whole_network/schedule/mapping  查看mapping说明按说明执行即可,ip修改为es地址即可
